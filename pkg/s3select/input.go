...
type SelectQuery interface {
	...
	runSelectParser(selectExpression string, myRow chan *Row)
	...
	//writeRecordMessage(payload string, currentMessage *bytes.Buffer) *bytes.Buffer
}
...
// NewInputJson sets up a new Input, the first Json is read when this is run.
...
// response writer in a streaming fashion so that the client can actively use
// the results before the query is finally finished executing. The
func (reader *JSONInput) Execute(writer io.Writer) error {
	myRow := make(chan *Row)
	curBuf := bytes.NewBuffer(make([]byte, 1000000))
	curBuf.Reset()
	progressTicker := time.NewTicker(progressTime)
